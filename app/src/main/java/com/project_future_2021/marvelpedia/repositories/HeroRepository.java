...
public class HeroRepository {
    ...
    //private final List<Hero> simpleRepoListOfHeroesTheUserSearchedFor = new ArrayList<>();
    ...
    public HeroRepository(Application application, String urlFromViewModel) {
        ...
        // if we already have data in the DB,
        //      we should re-fetch all these Heroes first, so that the users have up-to-date results.
        // else
    ...
    // heroApi is interface with x-methods. We need to implement all of these methods here,
    ...
    public void RepoLoadMore(Context context, String newUrl) {
    ...
    public List<Hero> RepoGetHeroesFromServer(Context context, String url, String requestTag, int offset, int limit) {
        ...
        Log.d(TAG, "RepoGetHeroesFromServer: url before making the call is: " + url);
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            ...
                            for (int i = 0; i < resultSize; i++) {
                                ...
                                // If he was, yes get the up-to-date data from the server from it,
                                // but also make the Hero favorite again.
                                ...
                                // that answer will be returned from different thread at a later point of time!
                            ...
                            // TODO should we add repoIsLoading.postValue(false); here too?
                            repoIsLoading.postValue(false);
    ...
    public List<Hero> RepoSearchForHeroesWithName(Context context, String url, String requestTag, String heroName) {
        ...
        url = url + "&nameStartsWith=" + heroName;
        // default value for limit by the server is 20, if we want to bring more/less results than that, we should specify it here.
        //url = url + "&limit=" + "50";


        Gson gson = new Gson();

        Log.d(TAG, "RepoSearchForHeroesWithName: url before making the call is: " + url);
                ...
                (Request.Method.GET, url, null, new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            ...
                            // If no heroes match the criteria the user gave,
                            // return an empty list.
                            if (resultSize == 0) {
                                ArrayList<Hero> emptyList = new ArrayList<>(0);
                                repoListOfHeroesTheUserSearchedFor.postValue(emptyList);
                                return;
                            }

                            final Hero[] temp_hero = new Hero[1];
                            Image temp_hero_thumbnail;
                            int temp_hero_id;
                            String temp_hero_name;
                            String temp_hero_description;
                            ...
                            for (int i = 0; i < resultSize; i++) {
                                ...
                                Log.d(TAG, "contains?-before: ");
                                int finalTemp_hero_id = temp_hero_id;
                                String finalTemp_hero_name = temp_hero_name;
                                String finalTemp_hero_description = temp_hero_description;
                                String finalTemp_hero_modified = temp_hero_modified;
                                Image finalTemp_hero_thumbnail = temp_hero_thumbnail;
                                String finalTemp_hero_resourceURI = temp_hero_resourceURI;
                                Comics finalTemp_hero_comics = temp_hero_comics;
                                Series finalTemp_hero_series = temp_hero_series;
                                Stories finalTemp_hero_stories = temp_hero_stories;
                                Events finalTemp_hero_events = temp_hero_events;
                                new AsyncGetHeroFromDb(db, new AsyncGetHeroFromDb.Listener() {
                                    @Override
                                    public void onResult(Hero resultHero) {
                                        if (resultHero != null && resultHero.getFavorite()) {
                                        ...
                                        temp_hero[0] = new Hero(finalTemp_hero_id, finalTemp_hero_name, finalTemp_hero_description, finalTemp_hero_modified, finalTemp_hero_thumbnail, finalTemp_hero_resourceURI, finalTemp_hero_comics, finalTemp_hero_series, finalTemp_hero_stories, finalTemp_hero_events, temp_hero_urls, temp_hero_favorite[0]);
                                        resultList.add(temp_hero[0]);
                                        //Log.d(TAG, "contains-inside: " + resultHero.getName() + resultHero.getFavorite());
                                        repoListOfHeroesTheUserSearchedFor.postValue(resultList);
                                    }
                                }).execute(temp_hero_id);
                                Log.d(TAG, "contains?-after: ");

                                //temp_hero[0] = new Hero(temp_hero_id, temp_hero_name, temp_hero_description, temp_hero_modified, temp_hero_thumbnail, temp_hero_resourceURI, temp_hero_comics, temp_hero_series, temp_hero_stories, temp_hero_events, temp_hero_urls, temp_hero_favorite[0]);

                                //Log.d(TAG, "contains?-hero about to enter with favorite: " + temp_hero[0].getFavorite());
                                //resultList.add(temp_hero[0]);
                            }
                            //repoListOfHeroesTheUserSearchedFor.postValue(resultList);
    ...
    public void deleteHeroWithId(int heroId) {
        HeroRoomDatabase.databaseWriteExecutor.execute(() -> {
            heroDao.deleteHeroWithId(heroId);
        });
    }
